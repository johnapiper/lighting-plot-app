...
// CAD model-space tools — annotations belong in Drawing/Sheet space, not the model
const CAD_TOOLS = [
  ...
  return (
    <div style={styles.toolbar}>
      ...
      <div style={styles.modeSwitcher}>
        <button
          title="CAD — model space: place fixtures, pipes, lines"
          style={{ ...styles.modeBtn, ...(activeMode === 'cad' ? styles.modeBtnActive : {}) }}
          onClick={() => onSetMode('cad')}
        >CAD</button>
        <button
          title="Drawing — sheet space: compose viewports, annotations, title block"
...
const styles = {
  toolbar: {
    ...
    padding: '4px 8px', gap: 4, flexShrink: 0, height: 48, overflowX: 'auto',
  },
  modeSwitcher: { display: 'flex', borderRadius: 5, overflow: 'hidden', border: '1px solid #0f3460', flexShrink: 0 },
  // Use `border` shorthand consistently — never mix with borderRight/Left etc. (React warns)
  modeBtn: { background: '#0d1b2a', border: '1px solid transparent', color: '#718096', cursor: 'pointer', padding: '5px 14px', fontSize: 11, fontWeight: 600, letterSpacing: '0.05em', transition: 'all 0.15s' },
