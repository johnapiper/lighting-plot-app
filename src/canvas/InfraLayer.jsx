...
// All symbol components render centred at (0, 0) in pixel space.
// The parent <g> handles translate to world position + scale(1/zoom).

function DistroSymbol({ sz, selected, item }) {
...
function NodeSymbol({ sz, selected, item }) {
  const r = sz * 0.9;
  return (
    <g>
      <rect x={-r} y={-r * 0.7} width={r * 2} height={r * 1.4} rx={4}
        fill="#1a1a2e" stroke={selected ? '#a78bfa' : '#7c3aed'} strokeWidth={selected ? 2 : 1.5} />
      <text x={0} y={r * 0.15} textAnchor="middle" fontSize={r * 0.65} fill="#a78bfa" fontWeight="bold">NODE</text>
      <text x={0} y={r * 0.65} textAnchor="middle" fontSize={r * 0.38} fill="#7c3aed">{item.label || 'Node'}</text>
    </g>
  );
}

function SwitchSymbol({ sz, selected, item }) {
...
function NetPortSymbol({ sz, selected, item }) {
  const r = sz * 0.65;
  return (
    <g>
      <rect x={-r} y={-r} width={r * 2} height={r * 2} rx={2}
        fill="#1a1a2e" stroke={selected ? '#6ee7b7' : '#10b981'} strokeWidth={selected ? 2 : 1.5} />
      <text x={0} y={r * 0.3} textAnchor="middle" fontSize={r * 0.9} fill="#10b981">🔌</text>
      {item.label && (
        <text x={0} y={r * 1.5} textAnchor="middle" fontSize={r * 0.7} fill="#10b981">{item.label}</text>
      )}
    </g>
  );
}
        ...
        return (
          <g key={item.id}
            ...
            <g transform={`scale(${1/zoom})`}>
              ...
              {item.onStructureId && (
                <circle cx={0} cy={0} r={SZ * 0.35}
                  fill="none" stroke="#e0c060" strokeWidth={1.5}
                  strokeDasharray="3 2" />
              )}
              <Sym sz={SZ} selected={selected} item={item} />
            </g>
          </g>
        );
      })}
